...
const Sidebar = ({ onAddPostClick, onSearchClick }) => {
  ...
  // Hide sidebar on the message page for mobile devices
  const isMessagePage = location.pathname === "/dashboard/message";

  return (
    <div
      className={`sidebar fixed h-full bg-black text-white md:w-[14%] w-full flex flex-col items-center md:py-4 md:px-[1rem] border-gray-800 ${
        isMessagePage ? "hidden md:flex" : "flex"
      }`}
    >
